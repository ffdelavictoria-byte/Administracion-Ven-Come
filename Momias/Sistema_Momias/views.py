...
def Asistencias_view(request):
    ...
    def obtener_monto_bloque(base_puesto, entrada, salida):
        ...
        try:
            ...
            if hrs < 0:
                hrs += 24  # Soporte para cruce de medianoche
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if estatus in ["Falta", "Permiso", "Vacaciones"]:
                ...
                salario_real = float(
                    puestos_salarios.get(puesto_seleccionado, 0)
                )
            ...
            def calcular_puntos(valor):
                ...
                if not valor or ":" in valor or "R1" in valor:
                    return 0
            ...
            if es_matutino and registros_dia.filter(
                entrada_matutina__isnull=False
            ).exclude(entrada_matutina='').exists():

                messages.error(
                    request,
                    "¡ERROR! Ya existe un registro para el TURNO MATUTINO de este empleado en esta fecha."
                )

                return redirect('asistencias')

            if es_vespertino and registros_dia.filter(
                entrada_vespertina__isnull=False
            ).exclude(entrada_vespertina='').exists():

                messages.error(
                    request,
                    "¡ERROR! Ya existe un registro para el TURNO VESPERTINO de este empleado en esta fecha."
                )
            ...
            asistencia.bonificacion = float(
                request.POST.get('bonificacion') or 0
            )

            asistencia.descuento = float(
                request.POST.get('descuento') or 0
            )

            asistencia.motivo_bonificacion = request.POST.get(
                'motivo_bonificacion'
            )

            asistencia.motivo_descuento = request.POST.get(
                'motivo_descuento'
            )

            asistencia.tipo_uniforme = request.POST.get(
                'tipo_uniforme'
            )

            asistencia.observaciones = request.POST.get(
                'observaciones'
            )
    ...
    registros_qs = Asistencia.objects.exclude(
        sucursal="FastFood"
    ).order_by('-fecha', '-id')
